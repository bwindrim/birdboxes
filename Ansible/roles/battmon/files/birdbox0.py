...
try:
    ...
    if len(initial_status) >= 2:
        client.publish("birdboxes/birdbox0/initial_status", int(initial_status[1], base=16), qos=1, retain=True)
    ...
    client.publish("birdboxes/birdbox0/startup_time", time.asctime(), qos=1, retain=True)
    ...
    client.publish("birdboxes/birdbox0/initial_battery_level", f'{level}%', qos=1, retain=True)
    ...
    client.publish("birdboxes/birdbox0/initial_stay_up", stay_up, qos=1, retain=True)
    client.publish("birdboxes/birdbox0/wake_time", timestr(wake_time), qos=1, retain=True)
    ntfy(f'BirdBox0 up at {timestr(now)}, for {stay_up} mins: batt {level}%')
    ...
    while stay_up > 0:
        ...
        client.publish("birdboxes/birdbox0/stay_up", stay_up, qos=1, retain=False)
        status = piwatcher_status()  # reset the watchdog
        level = getBatteryLevel(numReads=25)
        print("now = ", timestr(now), "stay up = ", stay_up, "battery level =", level, "status =", status)
        client.publish("birdboxes/birdbox0/battery_level", f'{level}%', qos=1, retain=True)
        if len(status) >= 2:
            status_val = int(status[1], base=16)
            if status_val != 0:
                client.publish("birdboxes/birdbox0/status", status_val, qos=1, retain=True)
...
#     piwatcher_wake(wake_time - now - 3) # set the wake-up interval
    ...
    client.publish("birdboxes/birdbox0/shutdown_time", time.asctime(), qos=1, retain=True)
    client.publish("birdboxes/birdbox0/wake_time", timestr(wake_time), qos=1, retain=True)
    ntfy(f'BirdBox0 down at {timestr(now)}, until {timestr(wake_time)}: batt {level}%, {message}')
